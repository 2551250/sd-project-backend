...
app.get("/TimeSpentByProjectByDate/:project_id", async (req, res) =>{
  ...
  ret = await query(
    `SELECT EMPLOYEE.EMPLOYEE_ID, NAME, SURNAME, EMPLOYEE_PROJECT.PROJECT_ID, DATE, ISNULL(SUM(DATEDIFF(MINUTE, START_TIME, END_TIME)) * 1.0 / 60, 0) AS TIME
    FROM dbo.EMPLOYEE_PROJECT INNER JOIN dbo.EMPLOYEE ON dbo.EMPLOYEE_PROJECT.EMPLOYEE_ID = dbo.EMPLOYEE.EMPLOYEE_ID LEFT JOIN dbo.TIMES ON EMPLOYEE.EMPLOYEE_ID = TIMES.EMPLOYEE_ID
    WHERE EMPLOYEE_PROJECT.PROJECT_ID = ${project_id}
    GROUP BY EMPLOYEE.EMPLOYEE_ID, NAME, SURNAME, DATE, EMPLOYEE_PROJECT.PROJECT_ID`
  );
  res.status(200).send(ret);
});
