...
app.get('/EmployeeProject/Employee/:id', async (req, res) => {
  ...
  ret = await query(
    `SELECT DISTINCT PROJECT_NAME, DESCRIPTION, ESTIMATED_TIME, MANAGER_ID, PROJECT.PROJECT_ID, EMP_PROJ_ID FROM dbo.PROJECT INNER JOIN dbo.EMPLOYEE_PROJECT ON (dbo.PROJECT.PROJECT_ID = dbo.EMPLOYEE_PROJECT.PROJECT_ID) INNER JOIN dbo.EMPLOYEE ON (dbo.EMPLOYEE.EMPLOYEE_ID = dbo.EMPLOYEE_PROJECT.EMPLOYEE_ID) WHERE dbo.EMPLOYEE.EMPLOYEE_ID = ${id};`
  );
  res.status(200).send(ret);
});
